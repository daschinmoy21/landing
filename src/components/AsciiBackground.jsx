...
export default function AsciiBackground() {
	...
	React.useEffect(() => {
		...
		const render = (time) => {
			...
			const flickerVal =
				CONFIG.animated && CONFIG.animStyle === 'flicker'
					? Math.sin(time * 0.015 * (CONFIG.animSpeed.intensity / 100)) *
			...
			const shimmerVal =
				CONFIG.animated && CONFIG.animStyle === 'shimmer' ? Math.sin(time * 0.005) * 20 : 0

			// Apply wave animation
			const waveShift = CONFIG.animated && CONFIG.animStyle === 'wave' ? time * 0.008 : 0
			...
			for (let row = 0; row < gridHeight; row++) {
				for (let col = 0; col < gridWidth; col++) {
					...
					if (shimmerVal) {
						lum +=
							Math.sin(col * 0.4 + row * 0.3 + time * 0.015) *
							(CONFIG.animIntensity.intensity / 450)
						lum = Math.max(0, Math.min(1, lum))
					}
			...
			cellsDrawn.forEach(({ x, y, lum, baseColor }) => {
				...
				const chromaIntensity = CONFIG.pfx.chromatic.enabled
					? CONFIG.pfx.chromatic.intensity / 22
					: 0
			...
			function drawShape(sx, sy, l) {
				...
				if (CONFIG.renderMode === 'dither' || CONFIG.renderMode === 'dots') {
					...
					const chars =
						CONFIG.charSet === 'binary' ? ['0', '1'] : (CONFIG.customChars || '.*+=#').split('')
